...
stagesep_pyro = stella.pyro3out
sustainer_pyro = stella.pyro4out
drogue_pyro = stella.pyro2out # Aft
main_pyro = stella.pyro1out # Forward
...
log = DataLogger(logfilename, [
    ...
    "bno_tilt",
    "hig_x", "hig_y", "hig_z", 
    "hig_gyro_x", "hig_gyro_y", "hig_gyro_z",
    "msg"
])
    ...
    while True:
            ...
            if state == _STATE_IDLE:
                ...
                if accelup < 0: # negative accel along thrust axis (not boosting anymore)
                    if stagenum == 1: # First Stage starts coast
                        maxalt = stella.bmp.altitude # reset
                        state = _STATE_COAST
                    elif stagenum == 2: # Upper Stage separates and starts ignition delay
                        # Stage Separation Charge
                        def stagesep_on():
                            stagesep_pyro.value = 1
                        def stagesep_off():
                            stagesep_pyro.value = 0

                        if isprimary:
                            stagesep_on()
                            newTimer(2, stagesep_off) # turn off after 2 seconds
                        else: # Backup
                            newTimer(1, stagesep_on) # turn on after 1 second
                            newTimer(3, stagesep_off) # turn off after 2 more seconds
                ...
                if time.monotonic() - gap_started > sus_ignition_delay:
                    if agl > sus_ignition_min_alt and vvel > sus_ignition_min_vel: # high enough, fast enough, and straight enough (TODO)
                        # Sustainer Ignition
                        def sustainer_on():
        ...
        if currenttime >= last_log + 0.1:
            ...
            try:
                ...
                hig_gyro = stella.hig.gyro
                log.values["hig_gyro_x"] = hig_gyro[0]
                log.values["hig_gyro_y"] = hig_gyro[1]
                log.values["hig_gyro_z"] = hig_gyro[2]
            except Exception as e:
            ...
            if can_tx:
                ...
                if radiotype == "nrfl":
                    ...
                    # SENSOR DATA               |        |     |        |      
                    data = struct.pack(">ffbbbbbffffffffffffbfbffffffffffffffffffff",
                        log.values.get("time", float('-inf')),
                        ...
                        log.values.get("bno_tilt", float('-inf')),
                        log.values.get("hig_x", float('-inf')),
                        log.values.get("hig_y", float('-inf')),
                        log.values.get("hig_z", float('-inf')),
                        log.values.get("hig_gyro_x", float('-inf')),
                        log.values.get("hig_gyro_y", float('-inf')),
                        log.values.get("hig_gyro_z", float('-inf'))
                        
                    )

                    # Header Format:
                    # Sender: "A" or "B"
                    # packet type: 1-Data, 2-Msg, 5-Handoff
